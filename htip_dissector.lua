...
--スパゲッティコードバージョン
--抽象と整理が必要！！
--ttcSubtype == 4 と　ttcsubtype == 5 不明
--出力データの型を書いてない　今は直接出力(ほとんど)

--20190118
--整理　抽象

--20190119
--シングルデータ、データ長とデータ、データリスト　の三パタンで半分整理した
--todo 
--type==127　のTVL名 直接subtypeの機能を展示する
--元LLDPフレーム部分の抽象
--プログラムの全体構造の最適化

--20190301
--tlv subtype name を書きました
--tlv head 部分整理
--データの一部を外に出す
--todo コード整理抽象
...
--20190304
--データを外に出す
--todo:string型データ最後の「00」の処理方法
--todo:コード整理　全体的に
--bug fix : chassisID TLV -> mac address (string -> hex)
...
--return data
function writeData(buffer, offset)
	local dataLen = buffer(offset, 1):uint()
	return buffer(offset + 1, dataLen)
end
--ttcSubtypeを書く
function eiIDNameWrite(eiID, buffer, offset)
	if(eiID == 1)       then return "indicator : "..writeData(buffer, offset):string()
	elseif(eiID == 2)   then return "maker code : "..writeData(buffer, offset):string()
	elseif(eiID == 3)   then return "model namne : "..writeData(buffer, offset):string()
	elseif(eiID == 4)   then return "model number : "..writeData(buffer, offset):string()
	elseif(eiID == 20)  then return "channel usage status : "..writeData(buffer, offset):uint()
	elseif(eiID == 21)  then return "radio wave intensity information : "..writeData(buffer, offset):uint()
	elseif(eiID == 22)  then return "communication error rate : "..writeData(buffer, offset):uint()
	elseif(eiID == 23)  then return "response time : "..writeData(buffer, offset):uint()
	elseif(eiID == 24)  then return "number of related devices : "..writeData(buffer, offset):uint()
	elseif(eiID == 25)  then return "number of active nodes : "..writeData(buffer, offset):uint()
	elseif(eiID == 26)  then return "radio quality : "..writeData(buffer, offset):uint()
	elseif(eiID == 27)  then return "number of retransmissions : "..writeData(buffer, offset):uint()
	elseif(eiID == 50)  then return "status information : "..writeData(buffer, offset):string()
	elseif(eiID == 51)  then return "CPU usage : "..writeData(buffer, offset):uint()
	elseif(eiID == 52)  then return "memory usage : "..writeData(buffer, offset):uint()
	elseif(eiID == 53)  then return "HDD usage : "..writeData(buffer, offset):uint()
	elseif(eiID == 54)  then return "battery level : "..writeData(buffer, offset):uint()
	elseif(eiID == 80)  then return "LLDP transmission interval : "..writeData(buffer, offset):uint()
	elseif(eiID == 255) then return "vendor proprietary extension area : "..writeData(buffer, offset)
	else return "unknow ttc subtype : "..writeData(buffer, offset):string()
	end
end
...
local ethaddr = Field.new("eth.addr")
--
function p_htip.dissector(buffer, pinfo, tree)
	...
	local htipTree = tree:add(p_htip, buffer(), "HTIP Protocol Data")
	...
	while(tlvType ~= 0) do
		...
		elseif(tlvType == 1) then--chassis id
			local tlvTree = htipTree:add(buffer(offset, tlvLen + 2), "Chassis ID TLV (Chassis ID:"..buffer(offset + 2, 1):uint()..", MAC Adress: " ..buffer(offset + 3, tlvLen -1)..")")
			...
			valueTree:add(buffer(offset + 3, tlvLen - 1), "MAC Adress: " ..buffer(offset + 3, tlvLen -1))
			
		elseif(tlvType == 2) then--prot id
			local tlvTree = htipTree:add(buffer(offset, tlvLen + 2), "Port ID TLV (Port ID subtype:"..buffer(offset + 2, 1):uint()..", port ID: " ..buffer(offset + 3, tlvLen -1):string()..")")
			writeHead(buffer, offset, tlvTree, tlvType, tlvLen)
			local valueTree = tlvTree:add(buffer(offset + 2, tlvLen), "TLV Information String")
			valueTree:add(buffer(offset + 2, 1), "port ID subtype: " ..buffer(offset + 2, 1):uint())
			valueTree:add(buffer(offset + 3, tlvLen - 1), "port ID: " ..buffer(offset + 3, tlvLen -1):string())
		...
		elseif(tlvType == 4) then--prot description
			local tlvTree = htipTree:add(buffer(offset, tlvLen + 2), "Port Description TLV (port description: " ..buffer(offset + 2, tlvLen):string()..")")
			writeHead(buffer, offset, tlvTree, tlvType, tlvLen)
			local valueTree = tlvTree:add(buffer(offset + 2, tlvLen), "TLV Information String")
		...
		elseif(tlvType == 127) then--本番
			...
			if(ttcSubtype == 1) then--機器情報
				...
				elseif(eiID == 3) then--機種名**
					setLenAndData(buffer, ttcEITree, temOffset, "model namne", "string")
...
do
	function packet_listener()
		local tap = Listener.new(nil, "eth.type == 0x88cc && eth.dst == ff:ff:ff:ff:ff:ff", false)
		
		function tap.packet(pinfo,tvb)
			local eth_table = DissectorTable.get("ethertype")
			eth_table:add(0x88cc, p_htip)
		end
